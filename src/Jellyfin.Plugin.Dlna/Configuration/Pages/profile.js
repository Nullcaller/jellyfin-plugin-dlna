...
function getProfile() {
    ...
    const id = '';
    const url = id ? 'Profiles/' + id : 'Profiles/Default';
...
function saveProfile(page, profile) {
    ...
    if (id) {
        ...
        ApiClient.ajax({
            type: 'POST',
            url: ApiClient.getUrl('Profiles'),
            data: JSON.stringify(profile),
            contentType: 'application/json'
        }).then(function () {
            Dashboard.navigate('configurationpage?name=profiles');
        }, Dashboard.processErrorResponse);
    }
...
export default function (view, params) {
    ...
    window.addEventListener('pageshow', function (_) {
        view.querySelector('#radioInfo').click();
        loadProfile(view);
    });
